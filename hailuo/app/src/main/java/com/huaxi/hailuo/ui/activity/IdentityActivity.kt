...
import com.huaxi.hailuo.idcard.IDCardScanActivity
import com.huaxi.hailuo.liveness.LivenessActivity
import com.huaxi.hailuo.liveness.util.ConUtil
import com.huaxi.hailuo.liveness.util.LegalityUtil
...
import com.huaxi.hailuo.presenter.impl.IdentityPresenter
...
class IdentityActivity : BaseActivity<IdentityContract.View, IdentityContract.Presenter>(), IdentityContract.View {

    private val TAG = "abc"

    override fun getLayout(): Int = R.layout.activity_identity

    override var mPresenter: IdentityContract.Presenter = IdentityPresenter()

    private var mMediaPlayer: MediaPlayer? = null
    private var isEditIdentity: String? = ""
    private var isCanEditIdentity: Boolean = false
...
//        val id_num = mIDCardBean.id_card_number
    ...
    override fun onOcrIdCardResult(data: IDCardBean) {
        if (data != null && data.legality != null) {
    ...
    override fun onCheckFaceResult() {
        ...
        if (!TextUtils.isEmpty(mWithdraw_money)) {
            val intent = Intent(mActivity, BindBankCardActivity::class.java)
            intent.putExtra("title", "绑定银行卡")
            intent.putExtra("money", mWithdraw_money)
