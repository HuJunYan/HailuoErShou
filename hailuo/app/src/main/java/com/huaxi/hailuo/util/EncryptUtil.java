...
public class EncryptUtil {
    ...
    public static void encryptPassword(JSONObject jsonObject) throws JSONException {
        List<String> keyList = SignUtils.copyIterator(jsonObject.keys());
        if (keyList.contains("password")) {
            jsonObject.put("password", MD5SHA1AndReverse(jsonObject.getString("password")));
        }
        if (keyList.contains("new_password")) {
            jsonObject.put("new_password", MD5SHA1AndReverse(jsonObject.getString("new_password")));
        }
    }
     ...
     */
    public static void encryptTwoPassword(JSONObject jsonObject) throws JSONException {
        List<String> keyList = SignUtils.copyIterator(jsonObject.keys());
        if (keyList.contains("password")) {
            jsonObject.put("password", MD5SHA1AndReverse(jsonObject.getString("password")));
        }
        if (keyList.contains("confirm_password")) {
            jsonObject.put("confirm_password", MD5SHA1AndReverse(jsonObject.getString("confirm_password")));
        }
    ...
    public static String SHA1MD5AndReverse(String Number) {
        ...
        String sha1number = EncryptUtil.getSHA1(Number).toLowerCase();

        StringBuffer sb = new StringBuffer(EncryptUtil.md5(sha1number));
...
//        Log.d("ret", "srcData = " + Number);
        String md5Number = EncryptUtil.md5(Number).toLowerCase();
//        Log.d("ret", "md5 = " + md5Number);
        String sha1Number = EncryptUtil.getSHA1(md5Number);
...
//        Log.d("ret", "reverse = " + key);
    ...
    public static String md5(byte[] param) {
        String returnStr;
        try {
            MessageDigest localMessageDigest = MessageDigest.getInstance("MD5");
