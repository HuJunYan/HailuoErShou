...
public class SpannableUtils {
     ...
     * @param text          要变化的文字
     * @param textSize      目标文字大小
     * @param spanTextColor 目标文字颜色
     */
    public static void setTextSizeSpan(TextView textView, String text, float textSize, int spanTextColor) {
        if (TextUtils.isEmpty(text)) {
            return;
        }
        String text1 = textView.getText().toString() + text;
        SpannableStringBuilder ss = new SpannableStringBuilder(text1);
        int start = text1.indexOf(text);
        int end = start + text.length();
        float proportion = textSize / textView.getTextSize();
        ss.setSpan(new RelativeSizeSpan(proportion), start, end, flag);
        setTransparentBackgoundSpan(ss, start, end);
        setSpanTextColor(ss, start, end, spanTextColor);
